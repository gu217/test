...
$sheet_size = count($data->sheets);
//print_r($data->sheets,FALSE);
for($sheet = 0; $sheet < $sheet_size; $sheet ++)
{
	$table_output [$sheet] = '';
	...
	for($row = 1; $row <= $data->sheets [$sheet] ['numRows'] && ($row <= $max_rows || $max_rows == 0); $row ++)
	{
		...
		{
			if (isset ( $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['colspan'] ) && $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['colspan'] >= 1 && $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['rowspan'] >= 1)
			{
			...
			else if (isset ( $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['colspan'] ) && $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['colspan'] >= 1)
			{
				$this_cell_colspan = " COLSPAN=" . $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['colspan'];
				$this_cell_rowspan = "";
				for($i = 1; $i < $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['colspan']; $i ++)
			...
			else if (isset ( $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['rowspan'] ) && $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['rowspan'] >= 1)
			{
				$this_cell_colspan = "";
				$this_cell_rowspan = " ROWSPAN=" . $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['rowspan'];
				for($i = 1; $i < $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['rowspan']; $i ++)
			...
			if (! empty ( $data->sheets [$sheet] ['cellsInfo'] [$row] [$col] ['dontprint'] ))
			{
